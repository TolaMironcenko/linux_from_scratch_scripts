...
mkdir lib
cd lib
for i in $(grep -v '^#' ../lib-7.md5 | awk '{print $2}');
do 
    busybox wget https://www.x.org/pub/individual/lib/$i;
done
...
sudo ln -sv $XORG_PREFIX/lib/X11 /usr/lib/X11 &&
sudo ln -sv $XORG_PREFIX/include/X11 /usr/include/X11
